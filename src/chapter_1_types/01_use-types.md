...
还有两种整数类型：有符号的[`isize`](https://doc.rust-lang.org/std/primitive.isize.html)和无符号的[`usize`](https://doc.rust-lang.org/std/primitive.usize.html)。其大小与目标系统上的指针大小相匹配。但是，您不会用 Rust 在指针和整数类型之间进行太多转换，因此大小等价并不重要。但是，标准库的集合类型将其大小作为 `usize`（来自 `.len()`）返回，因此集合索引意味着 `usize` 值非常常见——从容量角度来看，这显然是合理的，因为内存集合中的项目不能多于系统上的内存地址。
...
继续讨论Rust的基本类型。Rust 有 [bool](https://doc.rust-lang.org/std/primitive.bool.html) 类型、浮点类型（[f32](https://doc.rust-lang.org/std/primitive.f32.html)、[f64](https://doc.rust-lang.org/std/primitive.f64.html)）和 [unit 类型](https://en.wikipedia.org/wiki/Unit_type) ()（类似 C 的 void）。

更有趣的是 [`char`](https://doc.rust-lang.org/std/primitive.char.html) 字符类型，它保存一个 [Unicode值](https://www.unicode.org/glossary/#unicode_scalar_value)（类似于 Go 的 [`rune`类型](https://golang.org/doc/go1#rune)）。虽然它在内部存储为四个字节，但同样没有与32位整数的隐式转换。
...
当然，有一些辅助方法允许您在这些不同类型之间进行转换，但它们的签名会迫使您处理（或明确忽略）失败的可能性。例如，Unicode 代码点始终可以用 32 位表示，因此`'a' as u32`是允许的，但反过来就难办了（因为有些 u32 值不是有效的 Unicode 代码点）：
...
Rust 枚举特性的真正威力在于，每个变量都可以拥有随附的数据，使其成为充当[代数数据类型 (ADT)](https://en.wikipedia.org/wiki/Algebraic_data_type) 的聚合类型。主流语言的程序员对此不太熟悉；用 C/C++ 术语来说，它就像`enum`与`union`的组合——只是类型安全的。
...
但是，肯定还有其他罕见的情况需要使用 `Option<Vec<Thing>>` 来区分这两种情况 - 例如，加密系统可能需要区分“[单独传输的有效载荷](https://tools.ietf.org/html/rfc8152#section-4.1)”和“提供的空有效载荷”。 （这与 SQL 中关于列的 [NULL标记](https://en.wikipedia.org/wiki/Null_(SQL))的争论有关。）
